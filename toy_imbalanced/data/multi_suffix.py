...
# --- Candidate generator (4 candidates) ---
def generate_srs(stem, ending, stem_class):
    ...
    # ID-C alternation
    if stem_class.startswith("p"):
        alt1 = stem[:-1] + "w" + ending
    else:
        alt1 = stem[:-1] + "j" + ending

    alt2 = stem[:-1] + "l" + ending   # ID-lat
    alt3 = stem[:-1] + ending         # Max (deletion)

    return [canonical, alt1, alt2, alt3]
...
def write_tsv(endings, filename):
    ...
    with open(filename, "w", newline="", encoding="utf-8") as tsvfile:
        ...
        # --- constraint names ---
        writer.writerow(["", "", "", "*V-Obs-V", "Max", "ID-C", "ID-lat"])
        ...
        for sid, stem, stem_class in pairs:
            for ending in endings:
                ...
                if starts_with_vowel(ending):
                    if stem_class == "p-irregular":
                        winner_idx = 1
                    elif stem_class == "t-irregular":
                        winner_idx = 2
                ...
                for idx, cand in enumerate(candidates):
                    ...
                    writer.writerow([row_input, cand, perc, vc, mx, id_C, id_l])
